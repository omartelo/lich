...
- **项目与会话。** 项目位于顶部标签栏；`+` 可以重新打开你最近关掉的项目，其余的则调起
  ...
  Pull Request 的编号。
- **会话之间可以协作。** 一个会话可以把任务交给另一个会话，并拿回答案 —— 请求会被敲进
  那个会话的提示符，答案由它的智能体自己写回来，所以两张卡片里各跑着什么都不影响这件事
  成立。智能体既可以用它本就带着的工具够到其他会话（Claude Code 和 Codex 在启动时就被
  交予这些工具，opencode 和 Crush 则随插件获得），也可以用每个会话都携带的 `lich` 命令：
  `lich sessions`、`lich send "<卡片>" "<任务>"`、`lich reply`。它还能开一个新会话 ——
  这份工作值得的话，连同一个专属的 git worktree —— 并在结束时关掉它，同时决定那个检出
  目录的去留。右键点一张卡片，可以先把请求写到你自己的提示符上；请求还开着的时候，侧栏
  会在两端各自标出对方。同样这些命令在机器上任何一个 shell 里都能跑，`--json` 也在其中，
  所以脚本或定时任务不需要智能体参与也能驱动一个会话。完整的命令面见
  [`docs/cli.md`](docs/cli.md)。
- **内建 git worktree。** 从任意基础分支开一个 —— 本地或远程都能搜，哪怕有几十个分支
  —— lich 会把你被 gitignore 掉的 `.env*` 文件播种到新的检出目录里，派给它一个专属的
  开发服务器端口（`LICH_WORKTREE_PORT`）—— 既不与其他检出目录重合，也不会撞上机器上
  已经在监听的进程 —— 并在智能体启动前跑一遍按项目配置的初始化脚本。侧栏按会话所属的 worktree 分组，被保留下来的 worktree，其会话之后可以接着
  原来的对话恢复。
...
- **窗口里的其余部分。** `Ctrl`/`Cmd`+`K` 在会话与项目之间跳转 —— 既可以按名字，也可以
  ...
  [lich 插件](https://github.com/omartelo/lich-plugin)之后（在设置里装到能跑它的四个
  harness 中的任意一个），会话还会给自己的卡片起标题，并在它写入文件的那一刻刷新 git。
  设置 › 帮助可以打开日志文件夹和一份预填好的 bug 报告；`lich rage` 会把整份报告 ——
  版本、浏览器、providers、插件状态、日志，敏感值一律遮蔽 —— 打包成一个压缩包，
  `lich doctor` 则会说明 lich 在这台机器上究竟能不能启动。两者在窗口从未打开时也照样能用。
